...
class DQN(nn.Module):
    ...
    def __init__(self, hidden_size, obs_size, n_actions, sigma=0.5):
        ...
        self.fc_value = NoisyLinear(hidden_size, 1, sigma)
        self.fc_adv = NoisyLinear(hidden_size, n_actions, sigma)
...
class ReplayBuffer:
    ...
    def update(self, index, priority):
        if priority > self.max_priority:
            self.max_priority = priority.item()
        self.priorities[index] = priority.item()
...
class DeepQLearning(LightningModule):
    ...
    def __init__(self, max_steps, save_location, policy=greedy, capacity=100_000,
                 ...
                 b_start=0.4, b_end=0, b_last_episode=100, sigma=0.5):
        super().__init__()
        self.env = env
        self.n_actions = self.env.action_space.n

        obs_size = self.env.observation_space.shape[0]
        self.q_net = DQN(hidden_size, obs_size, self.n_actions, sigma=sigma)
    ...
    def save_reward_plots(self, step_rewards, episode_rewards, location):
        ...
        plt.figure()
        plt.title("Episode Rewards Noisy_DoubleDuelingDQN_prioritized")
        ...
        plt.figure()
        plt.title("Step Rewards Noisy_DoubleDuelingDQN_prioritized")
...
algorithm = DeepQLearning(1000, save_location='./Noisy_DoubleDuelingDQN_prioritized_results/')
